...
Response MultipartUploadStrategy::handleMultipartUpload(const HttpRequest& request)
{
    ...
    while (parseNextPart(ctx, headers, body))
    {
        ...
        {
            if (saveUploadedFile(headers.filename, body))
                savedFileCount++;
        }
    }
...
bool MultipartUploadStrategy::saveUploadedFile(const std::string& filename, const std::string& content) const
{
    ...
    std::string path = UPLOAD_DIRECTORY + safeFilename;
